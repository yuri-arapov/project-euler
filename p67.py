...
# cache is 2D array containing the sums that have been computed
# already, so we won't need to re-compute the same sequences
# over again
cache = [
  [ 0],
  [ 0,  0],
...
def maxsum(row, col):
        ...
        if s1 > s2:
                cache[row][col] = s1 + data[row][col]
        else:
                cache[row][col] = s2 + data[row][col]
